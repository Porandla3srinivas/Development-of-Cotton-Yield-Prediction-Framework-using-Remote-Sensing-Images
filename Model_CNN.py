...
def Model(X, Y, test_x, test_y, opt):
    ...
    model.fit(X, Y, epochs=50, steps_per_epoch=100, batch_size=32, validation_data=(test_x, test_y))
    pred = model.predict(test_x)
    return pred


def Model_CNN(train_data, train_target, test_data, test_target, opt=None):
    if opt is None:
        opt = 'adam'
    IMG_SIZE = 32
    ...
    pred = Model(Train_X, train_target, Test_X, test_target, opt)
    pred[pred >= 0.5] = 1
    pred[pred < 0.5] = 0
