...
# The improved  Hermit Crab Optimizer (HCO) -> update starts from line no 56
def Proposed(X, obj_function, lb, ub, num_iterations):
    ...
    for iter in range(num_iterations):
        ...
        for n in range(num_agents):
            if f[n] == 1:
                # Solitary search
                X[n, :] = P[n, :] + Pi[iter] / (min(Pi) + max(Pi) + (2 * Pi[iter])) * (B[n] - X[n, :])
            else:
                # Social search
                X[n, :] = S[n, :] + Pi[iter] / (min(Pi) + max(Pi) + (2 * Pi[iter])) * (P[(n + 1) % num_agents, :] - X[n, :])
