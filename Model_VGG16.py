...
def Model_VGG16(train_data, train_target, test_data, test_target, opt, sol=None):
    # VGG16
    if sol is None:
        sol= 50
    IMG_SIZE = [32, 32, 3]
    ...
    model.compile(
        ...
        optimizer=opt,
        metrics=['accuracy'])
    model.fit(x=Feat1, y=train_target, epochs=int(sol), steps_per_epoch=1)
    pred = model.predict(test_target)
    out = model.predict(Feat2)
    Eval = evaluation(out, test_target)
    return np.asarray(Eval)[:, 0], pred
